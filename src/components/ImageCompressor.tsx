...
function ImageCompressor() {
  ...
  const nameShorter = (file: File) => {
    if (file.name.length > 20) {
      return file.name.split("").splice(0, 12).join("") + "..." + file.name.split("").splice(-10).join("");
    }
    return file.name;
  };
  ...
  const handleImageCompression = async () => {
    if (blobs) {
      blobs.sort().forEach((image) => {
        image.status === "pending" &&
          new Compressor(image.file, {
            quality: controlsValue.quality > 1 ? 1 : controlsValue.quality < 0.1 ? 0.1 : controlsValue.quality,
            strict: true,
            success(result) {
            ...
            maxWidth: controlsValue.maxWidth < 50 ? 50 : controlsValue.maxWidth,
            maxHeight: controlsValue.maxHeight < 50 ? 50 : controlsValue.maxHeight,
            mimeType: controlsValue.type !== "auto" ? `image/${controlsValue.type}` : "auto",
            convertSize: 10000000,
            checkOrientation: true,
            convertTypes: ["image/jpeg", "image/png", "image/webp", "image/gif", "image/svg+xml"]
          });
      });
    }
...
-moz-box-shadow: -1px 0px 9px -1px #b1b1b1bf;
      ...
      await images.forEach((image) => {
        zip.file(`images/${image.compressed.name}`, image.compressed.file, {
          binary: true
        });
      });

      zip.generateAsync({ type: "blob" }).then((content) => {
        saveAs(content, `tinify.pro-${new Date().toISOString().slice(0, 10)}.zip`);
      });
    }
  ...
  useEffect(() => {
    if (images.length > 0) {
      const filtered = images.filter((img) => img.original.name === compare.original.name);
      filtered.length > 0 &&
        setCompare({
          original: filtered[0].original,
          compressed: filtered[0].compressed
  ...
  const imgHTML = images.map((image, index) => {
    return (
      <div className={styles.compressed} key={image.compressed.url}>
        ...
        >
          <img src={image.compressed.url} alt={image.compressed.name} />
        </div>
        <button
          ...
          onClick={() => {
            ...
            setImages((prevState: ImagesInterface[]) =>
              prevState.length > 1 ? prevState.filter((img) => img.compressed.url !== image.compressed.url) : []
            );
            setBlobs((state) => state.filter((img) => img.file !== image.original.file));
          }}
        >
          <RiDeleteBinLine />
        </button>
  ...
  const handleInstall = () => {
    setTimeout(() => {
      isSupported &&
        toast((t) => <InstallBanner t={t} handleInstallButton={handleInstallButton} />, {
    ...
    <Layout>
      ...
      <div className={styles.headerContainer}>
        ...
        >
          ...
          >
            Drag one or more files to this Drop Zone or Click here to select files
          </p>
          <input
            hidden
            ref={fileInputRef}
      ...
      <div className={styles.imagePreviewContainer}>
          ...
          <div>
            ...
            <AnimatePresence>
              <motion.div
                ...
                style={{
                  justifyContent: images.length === 1 ? "flex-start" : "space-around"
                }}
              >
                {images.length ? (
                  imgHTML
          ...
          <div className={styles.ToolsContainer}>
            ...
            <form>
              <div className={styles.formGroup}>
                <label className={styles.bold} htmlFor="quality">
                  Quality:
                </label>
                <input
                  type="number"
              ...
              <div className={styles.formGroup}>
                <label className={styles.bold} htmlFor="maxWidth">
                  Max Width:
                </label>
                <input
              ...
              <div className={styles.formGroup}>
                <label className={styles.bold} htmlFor="maxHeight">
                  Max Height:
                </label>
                <input
                  className={styles.textlike}
        ...
        <div className={styles.imagePreview}>
          ...
          {!compare.compressed.file && (
            <span className={styles.imagePreview__small}>Click on the thumbnail to view on the canvas</span>
          )}
            ...
            <ReactCompareSlider
              ...
              itemTwo={
                <div className={styles.afterContainer}>
                  <img src={compare.compressed.url} alt={compare.compressed.name} height="100%" width="100%" />
                  <div className={styles.imageInfo}>after</div>
                </div>
              }
              itemOne={
                <div className={styles.beforeContainer}>
                  <img src={compare.original.url} alt={compare.original.name} height="100%" width="100%" />
        ...
        <div className={styles.imageInfoContainer}>
          <div className={styles.imageDetailsHeader}>
            ...
            {compare.original.file ? (
              <>
                <div className={`${styles.imageInfo} ${styles.imageInfo__original}`}>
                  <h4>Original Image</h4>
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Time Stamp:</span>{" "}
                    <span className={styles.info}>
                      {new Date(compare.original.file.lastModified).toLocaleTimeString("en-US", {
                        hour12: false,
                        hour: "2-digit",
                        minute: "2-digit",
                        second: "2-digit"
                  ...
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>LastModified:</span>{" "}
                    <span className={styles.info}>
                      {new Date(compare.original.file.lastModified).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                        weekday: "short"
                  ...
                  <div className={styles.infoItem}>
                    ...
                    <span className={styles.info}>{nameShorter(compare.original.file)}</span>
                  </div>
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Type:</span> {compare.original.file.type}
                  </div>
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Size:</span> {(Number(compare.original.size) / 1000).toFixed(2)}
                    kb
                  </div>
                </div>
                <div className={`${styles.imageInfo} ${styles.imageInfo__compressed}`}>
                  <h4>Compressed Image</h4>
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Time Stamp:</span>{" "}
                    <span className={styles.info}>
                      {new Date(compare.compressed.file.lastModified).toLocaleTimeString("en-US", {
                        hour12: false,
                        hour: "2-digit",
                        minute: "2-digit",
                        second: "2-digit"
                  ...
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>LastModified:</span>{" "}
                    <span className={styles.info}>
                      {new Date(compare.compressed.file.lastModified).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                        weekday: "short"
                  ...
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Name:</span>{" "}
                    <span className={styles.info}>{nameShorter(compare.compressed.file)}</span>
                  </div>
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Type:</span> {compare.compressed.file.type}
                  </div>
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Size:</span> {(Number(compare.compressed.size) / 1000).toFixed(2)}
                    kb ({(100 - (Number(compare.compressed.size) / Number(compare.original.size)) * 100).toFixed(2)}%
                    Off)
                  </div>
                </div>
                <div className={`${styles.imageInfo} ${styles.imageInfo__compressed}`}>
                  ...
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Original Filesize:</span>{" "}
                    <span className={styles.info}>
                      {(
                        images.reduce((total, img) => {
                          return total + Number(img.original.file.size);
                        }, 0) / 1000000
                      ).toFixed(2)}
                      {"mb"}
                    </span>
                  ...
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Final Filesize:</span>{" "}
                    <span className={styles.info}>
                      {(
                        images.reduce((total, img) => {
                          return total + Number(img.compressed.file.size);
                        }, 0) / 1000000
                      ).toFixed(2)}
                      {"mb"}
                    </span>
                  ...
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>File Types:</span>{" "}
                    {Array.from(
                      new Set(
                        images.map((img) => {
                          return img.original.file.type.split("/")[1];
                        })
                      )
                    ).join(", ")}
                  </div>
                  <div className={styles.infoItem}>
                    <span className={styles.bold}>Reduced By</span>{" "}
                    {(
                      100 -
                      images.reduce((total, img) => {
                        return total + Number(img.compressed.file.size);
                ...
                <div className={styles.downloadContainer}>
                  <div className={styles.download}>
                    <a
                      ...
                      download={compare.compressed.file.name}
                      onClick={() => {
                  ...
                  <div className={styles.download}>
                    <button onClick={handleImageZip} className={styles.downloadButton}>
                      Download All
                    </button>
                  </div>
                </div>
